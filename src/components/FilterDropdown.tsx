...
const nameStyle = css("font:600 12px 'Public Sans',sans-serif");
const countStyle = css("margin-left:auto;font:500 10px 'JetBrains Mono',monospace;color:#b3ada2");
const tickStyle = css('color:#fff;font-size:9px;line-height:1');
        ...
        <>
          ...
          <div onClick={p.stop} style={panelStyle}>
            ...
            {p.filterChecks.map((f) => (
              <button key={f.name} data-tag={f.name} onClick={p.onToggleFilterChip} className="hv-bg" style={rowStyle}>
                <span style={css(f.checkStyle)}>{f.active && <span style={tickStyle}>✓</span>}</span>
